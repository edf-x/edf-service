...
import com.mk.eap.component.common.pdfbox.template.FilePathUtil;
 ...
 *
 */
public class CertificateTemplateFor3 implements TemplateStrategy {
	
	private final static Integer PAGE_SUM=15;//每页最多15条数据
	
	private final static Integer PAGE_TABLE_ROWS=5; //每个table 行数
	
	private final static Integer PAGE_TABLE_COUNT=3;//一页三个Table
	
	public CertificateTemplateFor3() {
	...
	private String getRealPath(TemplateTypeEnum templateType, int count){
		if(templateType == TemplateTypeEnum.A4B2){
			...
			if(count <=PAGE_TABLE_ROWS){
				return TEMPLATEPATH+"/"+"3editioncertificates-1.pdf";
			}else if(count <=PAGE_TABLE_ROWS*2){
				return TEMPLATEPATH+"/"+"3editioncertificates-2.pdf";
			}else if(count <=PAGE_TABLE_ROWS*3){
	 ...
	 */
	@Override
	public synchronized void operate(Map<String, Object[][]> dataMap, String srcFileName,TemplateTypeEnum templateType,Integer ... maxLineNum) throws IOException, PrinterException {

		Boolean isNotReturn = dataMap.keySet().size()==1;
		if(!isNotReturn) return;
		FillPdfTemplateUtil templateUtil = new FillPdfTemplateUtil();
		for (String doc : dataMap.keySet())
			try {
				{//几张单据
	 ...
	 * @param perDocInfo /每单的明细信息
	 * @param perRowCount 每单行数
	 * @param pageCount 生成的页数
	 * @throws IOException
	 * @throws PrinterException 
	 */
	private synchronized void buildPerContentAndSavePdf(String srcFileName, FillPdfTemplateUtil templateUtil, Object[][] perDocInfo,
			int perRowCount, int pageCount,Map<String, Object[][]> dataMap,TemplateTypeEnum templateType) throws IOException, PrinterException {
...
//			PDDocument pdfDocument = PDDocument.load(this.getClass().getResourceAsStream(getRealPath(templateType,objects.length)));
			...
			for (Object[] perRowColumns : objects) {
				...
				if (perRowColumns != null) {
					...
					perTableSumDr += Double.valueOf(perRowColumns[2]==null ? "0.00":  perRowColumns[2].toString()).doubleValue();
					perTableSumCr += Double.valueOf(perRowColumns[3]==null ? "0.00": perRowColumns[3].toString()).doubleValue();
				...
				if (iCount % PAGE_TABLE_ROWS == 0) {
					...
					if (iCount == PAGE_TABLE_ROWS) {// 说明是当前页第一个table
						...
						templateUtil.setSum(pdfDocument,
								...
								objects[10], tableCount == 1 ? "" : tablePercentAllTable);
						if (dataMap.keySet().contains(lastDocKeyAndValue.keySet().iterator().next())) {
							if (tableCount * PAGE_TABLE_ROWS == perDocAllCount) {
								perDocAllCount = 0;
							}
			...
			if (listInfo.size() == 1) {
				String path = FilePathUtil.SAVEFILE+FilePathUtil.SPLIT+FilePathUtil.createFile(FilePathUtil.getCurrentDate())+FilePathUtil.SPLIT+getOnlyKey()+".pdf";
...
//				pdfDocument.save(path);
				...
				listSteam.add(is);
//				String path = FilePathUtil.SAVEFILE+FilePathUtil.SPLIT+FilePathUtil.createFile(FilePathUtil.getCurrentDate())+FilePathUtil.SPLIT+getOnlyKey()+ i + ".pdf";
//				pdfDocument.save(path);
//				deleteList[i] = path;
//				pdfDocument.close();
			}
		...
		if (listInfo.size() > 1) {
			/*
			String[] newList = new String[listInfo.size() + 1];
			System.arraycopy(deleteList, 0, newList, 0, listInfo.size());
			String path = FilePathUtil.SAVEFILE + FilePathUtil.SPLIT
					+ FilePathUtil.createFile(FilePathUtil.getCurrentDate()) + getOnlyKey() + ".pdf";
			setReturnPdfPath(path);
			newList[listInfo.size()] = path;
			if (newList.length >= 3) {
				PDFMerger merger = new PDFMerger(newList);
//				merger.merge(newList);
				byte[] returnByteArray = merger.merge(newList);
...
//		PdfPrintEngineUtil printUtil = new PdfPrintEngineUtil(newList[listInfo.size()].toString());
	 ...
	 */
	@Override
	public synchronized void  batchOperate(Map<String, Object[][]> dataMap, String srcFileName,TemplateTypeEnum templateType,Integer ... maxLineNum) throws IOException, PrinterException {
...
//			int startIndex = docCount*PAGE_TABLE_ROWS;
				...
				for (int i = 0; i < pageCount; i++) {
					if (i + 1 == pageCount) {
						if (0 <(perDocDetailCount-i*PAGE_SUM) % PAGE_SUM &&(perDocDetailCount-i*PAGE_SUM) % PAGE_SUM <= 5) {
							...
							perDocinfo = new Object[PAGE_TABLE_ROWS * 2][];
						} else if((perDocDetailCount-i*PAGE_SUM) % PAGE_SUM > 10 && (perDocDetailCount-i*PAGE_SUM) % PAGE_SUM <= 15){
							perDocinfo = new Object[PAGE_SUM][];
		...
		Object[][] perDocInfo = totalContent;//每单的明细信息
